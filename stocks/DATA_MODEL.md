...
> 本文是 schema 参考索引，不记录项目阶段状态——阶段/完成度只在 `../STATUS.md`。
> 当前生产 schema 以 `stocks/domain/models.py` 和本文“当前生产模型（PRODUCTION）”章节为准；
> `AnalysisContext.schema_version` 当前为 `12`。
> 每个契约的生命周期标签（PRODUCTION/SHADOW/PLANNED/DEPRECATED）、实现文件和实际消费者
> 以 `../docs/contracts/README.md` 为准；本文只描述字段形状，不描述"是否已完成"。

## Shadow 契约（字段参考；生命周期状态见 `../docs/contracts/README.md`）

以下契约已实现并可运行，但仅产出 `.local/advisory_shadow/` 下的影子产物，不进入生产
推送、不写金融记忆。字段形状供实现参考；完成度和消费者以 `../docs/contracts/README.md`
为准，不在本节重复判断。
...
Deterministic human-readable presentation layer attached to every trading artifact. It is generated by `stocks/engine/presentation.py` and is the only part of `portfolio_decision` that an agent may render to the user. All natural-language names, public codes, Chinese action labels, estimated amounts, reasons, cash buckets, risk states, and research candidates are translated here; machine-only fields remain outside this view.

- `instruction_card.status`: `action_required | no_action | manual_review`
- `instruction_card.actions[]`: max 3 approved actions with `display_label`, `action_label`, `ratio`, `final_ratio`, `original_ratio`, `estimated_amount_cny`, `amount_is_estimate`, `reason_summary`, `decision_reason`, `evidence_summary`, `cancel_condition`, `settlement_display`, `settlement_rule`, `executable_quantity`, `execution_status`, `next_checkpoint`. These are all projected verbatim from the adjudicator's `PortfolioAction`; `presentation.py` performs no recomputation (no valuation × ratio, no estimate-flag derivation) and no fallback substitution (TASK-001D item 2/3).
- `assistant_brief`: `why`, `do_not_do`, `cash` (five canonical buckets — `available_now`/`confirmed_settling`/`planned_release`/`strategic_exit`/`locked`, each `{label, amount_cny}` — sourced verbatim from `portfolio_decision.cash_schedule`'s canonical fields of the same names; unresolved sale-settlement proceeds are excluded from every bucket and instead surfaced as a `data_notes` entry with an aggregate CNY amount, never a sixth bucket), `risk` (label from the fixed `hedge=对冲/高风险 | reduce=降风险 | watch=观察 | normal=常态` mapping; unknown levels fail closed to a review/unknown label rather than inventing prose; `transition_key` carries the raw enum alongside the rendered `transition` so render layers branch on state without comparing Chinese text), `research`.

Push agents must read only `portfolio_decision.user_view` for user-visible content; the rest of `portfolio_decision` is internal audit.

## DecisionEnvelope v1（DEPRECATED）

已废弃，不属于当前生产或目标架构，无生产/shadow 消费者。字段细节已移至
`../docs/contracts/legacy-reference.md`；生命周期状态权威见
...
- `portfolio_decision`：组合最终裁决，含 `status`、`approved_actions`、`suppressed_actions`、`replacement_chains`、`unresolved_conflicts`、`cash_schedule`。每个 `approved_action` 输出 `position_id`、`signal`、`ratio`、`action_description`、`cancel_condition`、`settlement_timing`、`next_checkpoint`，以及 `final_ratio`、`original_ratio`、`decision_reason`、`evidence_summary`、`settlement_rule`、`executable_quantity`、`execution_status`、`estimated_amount_cny`、`amount_is_estimate`（裁决层唯一权威来源，`user_view` 逐字投影，不重新计算）。
