...
- `instruction_card.actions[]`: max 3 approved actions with `display_label`, `action_label`, `ratio`, `estimated_amount_cny`, `amount_is_estimate`, `reason_summary`, `cancel_condition`, `settlement_display`, `next_checkpoint`.
- `assistant_brief`: `why`, `do_not_do`, `cash` (four natural buckets), `risk`, `research`.
