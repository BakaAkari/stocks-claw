...
def test_generator_script_is_repo_relative_and_branch_pinned():
    ...
    assert 'REPO_ROOT="${STOCKS_CLAW_REPO_ROOT:-/mnt/user/code-project/stocks-claw-trust-t1}"' in script
    assert 'REPO_ROOT="${STOCKS_CLAW_REPO_ROOT:-/mnt/user/code-project/stocks-claw}"' not in script
    assert 'EXPECTED_BRANCH="${STOCKS_CLAW_EXPECTED_BRANCH:-feat/decision-trust-t1}"' in script
...
def test_signal_settlement_wrapper_uses_project_venv():
    ...
    assert "stocks-claw-trust-t1" in script
