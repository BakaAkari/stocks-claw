...
> **2026-07-13 后续决策**: §9.6 叙事模板库（NarrativeBuilder）已被方案A否决。理由：6 个预定义叙事模板会导致输出固化；cluster 直接供 Agent 自行归纳更灵活。活跃设计文档 `NEWS_MODULE_REDESIGN.md` 已移除相关内容。
...
- [ ] 跨资产交叉验证
- [ ] 持续性判断 (读历史 snapshots)
- [x] ~~宏观叙事归纳~~ — 2026-07-13 否决。Cluster 直接供 Agent 归纳，不做中间叙事层
- [ ] 标题相似度去重
- [ ] 降级链补全
...
| 4 | ~~叙事构建完全不存在 — 碎片情报，无整合判断~~ → 方案A否决，cluster 直供 Agent 归纳 | 3/4 (已处理) | ~~🔴 高~~ |
...
- [ ] 主题→配置动作映射表
- [x] ~~叙事模板库 + NarrativeBuilder（5-8个常见叙事模板）~~ — 2026-07-13 方案A否决
...
### 9.6 叙事模板库（NarrativeBuilder）— ❌ 已否决 (2026-07-13)

> **否决理由**: 方案A。预定义叙事模板将复杂宏观环境强行归类，导致 Agent 输出固化。当前 cluster 的 theme/sentiment/summary 已足够供 Agent 自行归纳。详见 `NEWS_MODULE_REDESIGN.md` §7.3。
