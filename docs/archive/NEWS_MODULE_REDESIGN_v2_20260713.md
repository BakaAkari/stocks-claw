...
> 现行实现以 `ARCHITECTURE.md` 和代码为准；未来实施优先级以 `PLAN.md` 与 `EXECUTION_PLAN.md` 为准。

**版本**: v2.0（全新重写）
**日期**: 2026-07-13
**状态**: 历史设计稿，部分底层能力已实现；可复用结论已吸收到现行权威文档
**前置评审**: 四角色共识评审（量化/个人投资/金融预测/资产规划）；v1.1 评审稿已从工作树删除，仍可通过 Git 历史追溯。
